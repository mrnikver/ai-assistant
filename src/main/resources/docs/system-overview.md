...
- `RuntimeAgent` may invoke `getDeploymentStatus`, `getDeploymentLogs`, and guarded `restart_service` only. It cannot access RAG or treat mock state as documentation.
...
`restart_service(service, environment)` is a state-changing local/demo capability. The tool rejects malformed or unexpected arguments and targets outside the application allow-list (`orders-service` and `payments-service` in `dev` or `local`). A valid request creates an application-owned pending action containing an ID, conversation ID, tool name, complete validated arguments, status, and creation time, then returns a typed `CONFIRMATION_REQUIRED` result without executing it. An equivalent awaiting action in the same conversation is reused instead of creating another ID.

The typed result is terminal for the current orchestration request. `AgentRuntime` returns immediately from the Runtime Agent, the delegation boundary propagates the same terminal result, and the Supervisor also returns without another LLM call. The chat API exposes the application-owned response status and structured pending-action details; it does not rely on text or XML markers to identify this state.

An explicit follow-up confirmation is evaluated only from the caller's user message and is bound to the active pending action in that conversation. Agent prompts, tool observations, and generated text never enter this confirmation resolver. The application atomically transitions the stored action through `AWAITING_CONFIRMATION`, `CONFIRMED`, `EXECUTING`, and `EXECUTED`, runs the exact stored tool and arguments through `PendingActionExecutor`, and may then ask the Supervisor to summarize the actual result. Repeated, concurrent, already executed, or expired confirmations cannot execute the action again.
...
For guarded tools, application-owned execution context follows Supervisor delegation into the specialist registry. Pending-action traces record the action ID, tool, sanitized validated arguments, confirmation status, execution status, duration, and whether orchestration stopped for confirmation across request and confirmation turns. Prompt instructions are not an authorization boundary.
